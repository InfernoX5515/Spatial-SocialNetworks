...
class QueryInput:
    ...
    def __communityTimeInput(self):
        ...
        if self.gui.queryUser is not None and self.gui.summarySelected and self.gui.selectedRoadNetwork is not None and self.gui.selectedSocialNetwork is not None:
            ...
            dLabel = QtWidgets.QLabel(text="maximum number of hops(d): ")
            eLabel = QtWidgets.QLabel(text="minimum similarity threshold (η): ")
            tSLabel = QtWidgets.QLabel(text="start time of the search (tS): ")
            tELabel = QtWidgets.QLabel(text="end time of the search (tE): ")
            keyWeightLabel = QtWidgets.QLabel(text="weight of keywords (g): ")
            relWeightLabel = QtWidgets.QLabel(text="weight of relationships (h): ")
            poiWeightLabel = QtWidgets.QLabel(text="weight of POIs (m): ")
            # Create button
            ...
            self.queryWindow.pTextBox.setToolTip("m controls the weight for POIs for degree of similarity")
            # Create tS text box
            self.queryWindow.tSTextBox = QtWidgets.QDateEdit()
            self.queryWindow.tSTextBox.setMinimumDate(QtCore.QDate(2020, 1, 1))
            self.queryWindow.tSTextBox.setMaximumDate(QtCore.QDate(2020, 12, 31))
            self.queryWindow.tSTextBox.setDate(QtCore.QDate(2020, 1, 1))
            self.queryWindow.tSTextBox.setCalendarPopup(True)
            self.queryWindow.tSTextBox.setToolTip("tS controls the start range of time overlap")

            self.queryWindow.tETextBox = QtWidgets.QDateEdit()
            self.queryWindow.tETextBox.setMinimumDate(QtCore.QDate(2020, 1, 1))
            self.queryWindow.tETextBox.setMaximumDate(QtCore.QDate(2020, 12, 31))
            self.queryWindow.tETextBox.setDate(QtCore.QDate(2020, 12, 31))
            self.queryWindow.tETextBox.setCalendarPopup(True)
            self.queryWindow.tETextBox.setToolTip("tE controls the end range of time overlap")
            ...
            layout.addWidget(self.queryWindow.tSTextBox)
            layout.addWidget(tELabel)
            layout.addWidget(self.queryWindow.tETextBox)
            layout.addWidget(keyWeightLabel)
            layout.addWidget(self.queryWindow.kTextBox)
            layout.addWidget(relWeightLabel)
            layout.addWidget(self.queryWindow.rTextBox)
